...
get_vemco = function(filename, filepath = FALSE){
  proj_dir = getwd()
...
meta_data_col_names = function(data_frame){
  ...
  colnames(data_frame)[8]  <- '?'
  colnames(data_frame)[9]  <- 'fork_length(cm)'
  colnames(data_frame)[10] <- 'precaudal_length(cm)'
  colnames(data_frame)[11] <- 'cohort'
  colnames(data_frame)[12] <- 'area_of_capture'
  colnames(data_frame)[13] <- 'lat_deg'
  colnames(data_frame)[14] <- 'lat_min'
  colnames(data_frame)[15] <- 'lon_deg'
  colnames(data_frame)[16] <- 'lon_min'
  colnames(data_frame)[17] <- 'stomach_everted'
  colnames(data_frame)[18] <- 'eyes_popped'
  colnames(data_frame)[19] <- 'bladder_vented'
  colnames(data_frame)[20] <- 'point_of_incision'
  colnames(data_frame)[21] <- 'dna_clip'
  colnames(data_frame)[22] <- 'cannulation'
  colnames(data_frame)[23] <- 'sex'
  colnames(data_frame)[24] <- 'video'
  colnames(data_frame)[25] <- 'photo'
  colnames(data_frame)[26] <- 'photo_name'
  colnames(data_frame)[27] <- 'audio_log_file'
  colnames(data_frame)[28] <- 'dropshot'
  colnames(data_frame)[29] <- 'tissue_sample'
  colnames(data_frame)[30] <- 'gut_sample'
  colnames(data_frame)[31] <- 'tagger'
  colnames(data_frame)[32] <- 'notes'
  colnames(data_frame)[33] <- 'recaptured'
  colnames(data_frame)[34] <- 'detections'
  colnames(data_frame)[35] <- 'comments'
  return(data_frame)
}

## Usage
load_tagging_meta = function(filename, filepath = FALSE){
...
clean_vue_lat_lon = function(vue_data_df, receiver_data_df){
  for (a in 1:length(vue_data_df$datetime)){
    for (i in 1:length(receiver_data_df$station_name)){
      if (vue_data_df$receiver[a] == receiver_data_df$vr2w_serial[i] & 
            vue_data_df$datetime[a] >= receiver_data_df$deployment_date[i] &
            vue_data_df$datetime[a] < receiver_data_df$recovery_date[i]){
        vue_data_df$lat[a] = receiver_data_df$lat[i]
        vue_data_df$lon[a] = receiver_data_df$lon[i]
        vue_data_df$station[a] = receiver_data_df$station_name[i]
      }
    }
  }
}
...
remove_location = function(vue_data, location_to_remove = FALSE){
  if (location_to_remove == FALSE){
    ...
    keep_data = vue_data[!vue_data$location %in% location_to_remove,]
    return (keep_data)}
}

### clean_vue -> Removing database enteries not associated with tag ids ------------------
clean_vue = function(vue_data, bottomfish_tag_ids = FALSE){
...
generate_tagging_detection = function(tagging_data_df, vue_df){
  ...
  vue_data = vue_data[order(vue_data$receiver), ] # first reorders by receiver
  vue_data = vue_data[order(vue_data$tag_id), ] # then reorders by tag
  vue_data = vue_data[order(vue_data$datetime), ] # then reorders by date
  return (vue_data)
  }
